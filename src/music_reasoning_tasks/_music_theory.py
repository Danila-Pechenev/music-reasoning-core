...
CHOICE_TAILS = (
    "Answer format: one label from this list: {options}.",
    "Choose one label from: {options}.",
    "Use one label from these options: {options}.",
    "The expected answer is exactly one of these labels: {options}.",
    "Select one label from: {options}.",
)
YES_NO_TAILS = (
    ...
    "Respond with one lowercase word: 'yes' or 'no'.",
)
INTEGER_TAILS = (
    "Answer format: one integer.",
    "Give one integer.",
...
class NoteRenderer:
    ...
    def answer_format(answer_notation: str, with_octave: bool | None = None, explicit_accidentals: bool = False) -> str:
        ...
        if answer_notation == "compact ABC notation" and with_octave:
            answer_format = "one full compact ABC pitch token, for example, ^C, _B,, or =F'"
        ...
        if answer_notation == "scientific pitch notation":
            example = "C#4 or Bbb2" if with_octave else "C# or Bbb"
        else:
            example = "^C, _B, or =F"
        answer_format = f"one note name{octave_phrase} in {answer_notation}, for example, {example}"
        if explicit_accidentals:
            answer_format += ", with any accidental made explicit"
        return answer_format
