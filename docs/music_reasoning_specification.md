...
The remaining families below are planned implementation targets. They are kept
in this document so the repository remains a coherent long-term generator plan,
not only a description of the first two modules.
...
The package API mirrors the Reasoning Core task interface:
...
## Implemented Notation and Answer Policies

The first two implemented families share these notation styles:

| Style | Meaning | Example |
|---|---|---|
| `spn` | Scientific pitch notation | `C#4`, `Bbb2`, `F` |
| `compact_abc` | Compact ABC note tokens, quoted in prompts/COT when used inline | `"^C"`, `"_B,,"`, `"=F'"` |
| `full_abc_score` | Short ABC fragments with `L:`, `M:`, and `K:` headers | `L:1/8` / `M:2/4` / `K:Eb` / `[CEG]` |

The probability of ABC notation is controlled by `p_abc`. Conditional on ABC
being selected, 70% of cases use compact ABC tokens and 30% use full ABC score
...
that accidental explicitly. For example, the key signature `K:Bb` makes `B`
flat, so the score note `B` represents `"_B"`.

Note-answer modes do not ask the model to return a full ABC score. If the
prompt style is SPN, the answer is in scientific pitch notation. If the prompt
style is compact ABC or full ABC, the answer is in compact ABC notation.
Full-score ABC answer cases require explicit accidentals, including naturals
such as `=F`.
...
answers, final periods, whitespace variants, optional surrounding quotes,
optional leading articles such as `a` or `an`, and conventional Roman-numeral
aliases such as `V2/bVII` for `V42/VII` and `#iv°64` for `vii°64/V`. It also
accepts equivalent compact ABC octave spellings such as `__D'` and `__d`,
Unicode figured-bass digits such as `iii⁴₃` for `iii43`, explicit root-position
triad figures such as `III53` for `III`, `root position 7/5/3` for
`root position 7`, and `third inversion 6/4/2` for `third inversion 4/2`, while
still requiring spaces between words. It does not treat `C#` and `Db` as equal
unless the task itself asks for enharmonic equivalence.
...
- `mode == "any"` samples uniformly from the eight implemented modes.
- `interval_arithmetic` uses three operation types: add, subtract, and
  reduce-to-simple-then-invert. At higher levels, these appear in longer chains.
- `interval_construction` samples three answer-shape cases: prompt with octave
  and answer with octave; prompt with octave and answer without octave; prompt
...
**Tools:** project-owned pitch/interval spelling solver as source of truth;
custom rule tables for transposing instruments; `music21` verification in
tests for independent cross-checks of pitch and interval behavior.
...
- `mode == "any"` samples uniformly from the eight implemented modes.
- Chord-tone prompts may be rendered in SPN, compact ABC, or full ABC score
  fragments. Full ABC chord prompts use bracketed chord events and require
  key-signature interpretation before applying the chord/Roman rule.
- Unordered chord-tone collection modes usually omit octaves and may shuffle
  chord-tone order. Ordered or register-sensitive modes use octave-bearing
  chords when the order or voicing is musically meaningful.
- `open_close_voicing` always includes octaves, because voicing depends on
  register.
- `chord_from_roman_numeral` has no input chord to render. Its requested answer
  notation follows the sampled style: SPN prompts ask for SPN note sequences,
  while compact/full ABC styles ask for compact ABC note sequences. Full ABC
  answer cases require explicit accidentals.
...
**Tools:** `music21.chord`, `music21.roman`, and `music21.key` for
convention-heavy chord/Roman operations; custom rendering, normalization,
ABC-handling, chromatic-label, and reasoning-trace layers in this package.
...
- `reasoning_core`: task/config/problem interface, generation protocol, and
  scoring entry points.
- `music21`: symbolic music-theory engine. The current implementation uses it
  directly for chord/Roman-numeral generation and uses it in tests to verify
  pitch/interval behavior.

## Current Test/Development Dependencies

- `pytest`
- `pytest-cov`
- `ruff` for development environments
